...
## x402 — BSV Micropayment Client for Codex
...
When the user's task matches this skill, follow these steps exactly.
...
All scripts are in this skill's directory. Run them with `python3`.

**IMPORTANT:** The script path is relative to this skill's installation directory.
Find the skill directory first, then run scripts from there.

#### List all agents
```bash
python3 <skill-dir>/scripts/brc31_helpers.py list
```
Shows all registered agents with names, descriptions, and URLs.

#### Discover an agent's capabilities
```bash
python3 <skill-dir>/scripts/brc31_helpers.py discover <agent-name>
```
Fetches the x402-info manifest — endpoints, schemas, pricing, error codes.
Agent names: `banana`, `veo`, `whisper`, `x-research`, `nanostore`.

#### Get wallet identity
```bash
python3 <skill-dir>/scripts/brc31_helpers.py identity
```
Returns the user's public identity key from MetaNet Client.

#### Authenticated request (no payment)
```bash
python3 <skill-dir>/scripts/brc31_helpers.py auth <METHOD> <agent/path> [body-json]
```
Makes a BRC-31 authenticated request. Handles handshake automatically.

#### Paid request (auth + payment)
```bash
python3 <skill-dir>/scripts/brc31_helpers.py pay <METHOD> <agent/path> [body-json]
```
Makes an authenticated request with automatic 402 payment handling:
1. Sends initial request → server returns 402 with price
2. Creates payment transaction via MetaNet Client wallet
3. Retries with `x-bsv-payment` header → server accepts, returns result
...
```bash
# Generate an image
python3 <skill-dir>/scripts/brc31_helpers.py pay POST banana/generate '{"prompt":"a mountain sunset","aspect_ratio":"16:9"}'

# Search Twitter
python3 <skill-dir>/scripts/brc31_helpers.py pay POST x-research/search '{"query":"bitcoin scaling","max_results":20}'

# Transcribe audio (base64-encoded)
python3 <skill-dir>/scripts/brc31_helpers.py pay POST whisper/transcribe '{"audio":"<base64>","language":"en"}'

# Free authenticated endpoint
python3 <skill-dir>/scripts/brc31_helpers.py auth POST banana/free
```
...
- **MetaNet Client not running:** Tell the user to start MetaNet Client from `/Applications`
...
- **Auth failure (401/403):** Try `python3 <skill-dir>/scripts/brc31_helpers.py handshake <server-url>` to reset the session
...
For advanced use, the full CLI is at `<skill-dir>/cli.py`:
```bash
python3 <skill-dir>/cli.py --help
python3 <skill-dir>/cli.py list
python3 <skill-dir>/cli.py auth POST banana/free
python3 <skill-dir>/cli.py pay POST banana/generate '{"prompt":"a cat"}'
python3 <skill-dir>/cli.py session --clear-all
```
